class Auto {
    constructor(id, nome, anno, carburante, colore, km, origine, vel, foto, prezzo) {//dev'essere scritto cosi, è una funzione all'interno del quale dobbiamo insierire proprietà ed associazioni
        this.id = id;
        this.nome = nome,
        this.anno = anno;
...
var veicoli = [];// creazione dell'array all'interno del quale pusho le auto
var select;//qui la inizializzo per uno scopo globale
...
function populateSelectOptions() {
    ...
    select.addEventListener("change", stampaAuto)//all'interno di un addeventlistener non vanno le parentesi tonde

    for (i = 0; i < veicoli.length; i++) {

        var option = document.createElement("option")//option di html è richiamato
        option.setAttribute("value", veicoli[i].id);//aggiunge un nuovo attributo (value in questo caso)
...
function stampaAuto() {
    ...
    try {
        ...
        document.querySelector("#immagineAutoSelezionata").setAttribute("src", "imgauto/" + autoSelezionata.foto)
